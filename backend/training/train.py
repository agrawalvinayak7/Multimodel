...
def main():
    ...
    # transfering the model and all the tensors to the devive that we get cpu or the gpu
    model = MultimodalSentimentModel().to(device)

    trainer = MultimodalTrainer(model, train_loader, val_loader)

    # as we are training the model if the current val loss is better than the last checkpoint we'll go ahead and save the model
...
# tqdm is used to see progress bar in the terminal while running the program
...
# Tracking the gpu usage during each epoch
        if torch.cuda.is_available():
            memory_used = torch.cuda.max_memory_allocated() / 1024**3
